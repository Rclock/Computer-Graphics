...
Box can be changed to different colors and compare coordinates to itself.
...
/**
 \brief
 Important function for determining if the given x and y coords are inside the box or not. This is helpful for both the mouseover
 and the drag functions. Returns true if the coordinates given are over the box.

 \param x - X coordinate, converted to screen coords
 \param y - y coordinate, converted from pixel to screen coords
 */
 bool Box::compareToBox(GLfloat x, GLfloat y)
 {
 std::cout << "Box Width: " << width << "Box Height: " << height << "\n";
    ...
    //move ui color change 4 lines to here
return false;
 }
 /**
  \brief
  This function changes the color of the box to red, and takes a boolean to determine if we need to switch it to red or revert it
  back to it's original color. This is determined elsewhere in the project.

   \param red - A bool. If red is true, the box vertices will now be red. If it is false, the box vertices will go back to
   what color it was before depending on the booleans of the other colors.
   */
 void Box::changeColor(bool red)//make a similar function with the mouse over!! inside the box!
 {
