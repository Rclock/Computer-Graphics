...
TextRendererTTF::TextRendererTTF(std::string fontFile)
{
    ...
    std::string fragShade =
        ...
        "out vec4 col;\n"
        "void main(void) {\n"
        "  col = vec4(1, 1, 1, texture(tex, texpos).r) * color;\n"
        "}\n";
    ...
    if(uniform_tex == -1 || uniform_color == -1 || rotmat == -1)
    {
...
{
    if (fontLoadError || shaderError || freetypeError)
    {
        if (fontLoadError)
...
/**
\brief Loads in the the size of the screen.

Loads in the the size of the screen, needed before text rendering so that the
x and y scaling of the font are correct and correspond to actual pixel positions and
sizes.
...
void TextRendererTTF::setScreenSize(GLuint w, GLuint h)
{
    ScreenWidth = w;
    ScreenHeight = h;
}
...
\param xpos --- Horizontal pixel position for the lower left corner of the text box.
\param ypos --- Vertical pixel position for the lower left corner of the text box.
\param rot --- Optional, counter clockwise degree rotation of the text around the
(xpos, ypos) position.

\note The setScreenSize(w, h) method must be called before this method for the
positions and sizes to correspond to pixels.

*/

void TextRendererTTF::draw(std::string text, GLuint xpos, GLuint ypos, GLfloat rot)
{
    draw(text.c_str(), xpos, ypos, rot);
}
...
void TextRendererTTF::draw(const char* text, GLuint xpos, GLuint ypos, GLfloat rot)
{
    if (isError())
        return;
    ...
    GLfloat sx = 2.0/ScreenWidth;
    GLfloat sy = 2.0/ScreenHeight;
    GLfloat x = 2.0*((GLfloat)xpos / ScreenWidth) - 1;
    GLfloat y = 2.0*((GLfloat)ypos / ScreenHeight) - 1;

    const char *p;
    FT_GlyphSlot g = face->glyph;

    GLfloat pi_180 = 0.0174532925199432957692369076849;
    glm::mat4 RotationMatrix = glm::mat4(1.0);
    RotationMatrix = glm::translate(RotationMatrix, glm::vec3(x, y, 0));
    RotationMatrix = glm::scale(RotationMatrix, glm::vec3(sx, sy, 1));
    RotationMatrix = glm::rotate(RotationMatrix, rot*pi_180, glm::vec3(0, 0, 1));
    RotationMatrix = glm::translate(RotationMatrix, glm::vec3(-x, -y, 0));
