...
void YPRCamera::addRoll(float num)
{
    ...
    rolldegree += num;
    if (fabs(rolldegree) > 360)
        rolldegree = 0;
    // Rotate the up vector.
    Up4 = rot*Up4;

// transformed up vector to 3D.
    Up = glm::vec3(Up4);
...
glm::mat4 YPRCamera::getWorldTransform()
{
    glm::vec3 right = glm::cross(View, Up);

    glm::vec4 R = glm::vec4(-right, 0);
    glm::vec4 V = glm::vec4(View, 0);
    glm::vec4 U = glm::vec4(Up, 0);
    glm::vec4 T = glm::vec4(Position, 1);

    glm::mat4 m = glm::mat4(R, U, V, T);

    return m;
}

glm::vec3 YPRCamera::getYPRdegrees()
{

    glm::vec3 degrees;
    degrees.x = yawdegree;
    degrees.y = pitchdegree;
    degrees.z = rolldegree;
