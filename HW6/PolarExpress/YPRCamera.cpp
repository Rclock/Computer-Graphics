...
\brief Implementation file for the YPRCamera class. Includes implementation of railroad camera.
...
/**
\brief Sets the position of the roller coaster camera on the track. Uses the function given and it's derivative. Calls
setView, setPosition, and setUpViaY to keep the up vector and view vector up to date.

\param theta --- Current angle to be plugged into the functions to position on the track. Locked to frame rate.

*/
void YPRCamera::setRailroadCamera(float theta)
{
    float posx, posy, posz, dposx, dposy, dposz;
    posx = 9.8*cos(theta);
    posy = sin(3*(theta)) - 2*cos(2*(theta + 0.2)) + 2 *sin(7*(theta)) + .15;
    posz = 9.8*sin(theta);

    setPosition(posx, posy, posz);
    //derivative
    dposx = -1*9.8*sin(theta);
    dposy = 3*cos(3*theta) + 4*sin(2*(theta + 0.2)) + 14*cos(7*theta);
    dposz = 9.8*cos(theta);

    setView(dposx, dposy, dposz);

    //disregard, tried to manually set the 3 vectors but I set the "up" vector to point perpindicular to where I wanted it to.
    //glm::vec3 pos(9.8*cos(theta), sin(3*(theta)) - 2*cos(2*(theta + 0.2)) + 2 *sin(7*(theta)) + .1, 9.8*sin(theta));
    //glm::vec3 view(-1*9.8*sin(theta) + frame, 3*cos(3*theta) + 4*sin(2*(theta + 0.2)) + 14*cos(7*theta) + frame, 9.8*cos(theta) + frame);

    //yprcamera.setUp(9.8*cos(theta), sin(3*(theta)) - 2*cos(2*(theta + 0.2)) + 2 *sin(7*(theta)) + .1, 9.8*sin(theta));
